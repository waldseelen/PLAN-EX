...
import { usePlanner, usePlannerApp, usePlannerHabits } from '../../store'
...
export function SettingsModal() {
    const { isSettingsOpen, setIsSettingsOpen, settings, updateSettings, addToast, backupWarning } = usePlannerApp()
    const { courses, completionState, personalTasks, lectureNotesMeta, importData } = usePlanner()
    const { habits, importHabits } = usePlannerHabits()
    ...
    const handleExport = async () => {
        setIsExporting(true)
        try {
            const _habitLogs = await getAllHabitLogs()
            ...
            const backup: BackupData = {
                ...
                habits: habits,
                settings: settings,
                lectureNotesMeta: lectureNotesMeta,
            }
